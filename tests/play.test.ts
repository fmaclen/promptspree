...
test.describe('Play', () => {
	...
	test.describe('Logged in', () => {
		...
		test('Can generate draft articles', async ({ page }) => {
			...
			await matchSnapshot(page, 'chat-empty');
			...
			await expect(
				page.locator('div.chat__message--assistant a.category', {
					hasText: MOCK_ARTICLE_COMPLETIONS[0].category
				})
			).toBeVisible();
		...
		test('Can publish an earlier version of a draft article', async ({ page }) => {
			...
			await matchSnapshot(page, 'chat-with-article-versions');
