...
test.describe('Play', () => {
	...
	test("Can't play, user is not logged in", async ({ page }) => {
		...
		await expect(page.getByText('textarea')).not.toBeVisible();
	});
	...
	test.describe('Logged in', () => {
		...
		test('Can generate draft articles', async ({ page }) => {
			...
			let prompt = MockPrompt.GENERATE_ARTICLE_0;
			...
			expect(await page.screenshot({ fullPage: true })).toMatchSnapshot({ name: 'chat-empty.png' , maxDiffPixelRatio: MAX_DIFF_PIXEL_RATIO }); //prettier-ignore
			...
			expect(articleMessages?.[0].content).toBe(MockPrompt.GENERATE_ARTICLE_0);
			...
			expect(articleMessages?.[2].content).toBe(MockPrompt.GENERATE_ARTICLE_1);
		...
		test('Can publish lastest version of a draft article', async ({ page }) => {
			...
			await page.locator('textarea').fill(MockPrompt.GENERATE_ARTICLE_0);
			await expect(page.getByText(MOCK_ARTICLE_COMPLETIONS[0].headline)).not.toBeVisible();
		...
		test('Can publish an earlier version of a draft article', async ({ page }) => {
			...
			const generateButton = page.locator('button.chat__button-generate');
			const publishButton = page.locator('button.form-button', { hasText: 'Publish' });
			await page.locator('textarea').fill(MockPrompt.GENERATE_ARTICLE_2);
			...
			await expect(page.getByText(MOCK_ARTICLE_COMPLETIONS[2].headline)).toBeVisible();
			await expect(publishButton).toBeVisible();

			// Second prompt
			await page.locator('textarea').fill(MockPrompt.GENERATE_ARTICLE_3);
			await generateButton.click();
			await expect(page.getByText(MOCK_ARTICLE_COMPLETIONS[2].headline)).toBeVisible();
			await expect(page.getByText(MOCK_ARTICLE_COMPLETIONS[3].headline)).toBeVisible();
			expect(await page.screenshot({ fullPage: true })).toMatchSnapshot({ name: 'chat-with-article-versions.png' , maxDiffPixelRatio: MAX_DIFF_PIXEL_RATIO }); //prettier-ignore
