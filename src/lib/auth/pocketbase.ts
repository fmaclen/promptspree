...
// 		if (!('user' in resp.value)) return err(new Error(resp.value?.message ?? 'no user found'));
...
// 		if (!('id' in user) || !('email' in user)) return err(new Error('no user found'));
...
// 	return ResultAsync.fromPromise<T, Error>(request, () => new Error(fallback_error_message));
// }
